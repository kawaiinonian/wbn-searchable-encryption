...
def measure_size():
    ...
    for k, v in DictW.items():
        dictwsize += len(k) + len(v)
    total_size = Omapsize + alistsize + dictwsize
    print("acount for {} {} {}".format(Omapsize/total_size, alistsize/total_size, dictwsize/total_size))
    return total_size
...
for n in num:
    ...
    for i in tqdm(range(1000)):
        ...
        du = user(uid)
        ukey = do.enroll(uid)
        enroll(ukey, int(n*10*0.05))
        du.register(ukey)
        for d in doc[:int(n*0.05)]:
            do.share(du.uid, d, documents[d])
