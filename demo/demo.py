...
for k, v in document.items():
    files.append(get_fd(v, usr1+k.ljust(LAMBDA)))
...
query_ = cusr.search_generate(get_word_from_bytes(word), uk1, dockey, [])
query_ = [(bytes(q.uid), bytes(q.stk)) for q in query_]
...
result = cserver.search(query, bytes(aset2.contents.aid), uset, aset, xset)
print("finish search on server, find %d"%len(result))
for r in result:
